...
function GatherData() {
    const { mergeRequestList, notesList } = useAuth();
        ...
        if (mergeRequestList !== 0) {
            for (let [user, uservalue] of Object.entries(mergeRequestList)) {
                ...
                for (let [key, value] of Object.entries(uservalue['mr'])) {
                    ...
                    for (let [keylines, valuelines] of Object.entries(value['lineCounts'])){
...
export function ScoreCalculator(username) {
    ...
    for (let i=0; i<barData.length; i++){
        ...
        if (username === data['name']){
            TotalScore =  parseInt(data['commits']) + 
                parseInt(data['code']) + 
                parseInt(data['issue']) + 
                    ...
                    {barData.map((Detail, index) => {
                        return (
                            <div className="scoreArray">
                                <div className="user" style={{color: usercolours[index]}}>@{Detail.name}</div>
