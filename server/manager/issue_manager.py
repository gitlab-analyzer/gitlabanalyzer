...
class IssueManager:
    ...
    def get_issues_by_date_range(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Tuple[Issue]:
        startDate: Optional[datetime] = parser.parse(start_date) if start_date is not None else None
        endDate: Optional[datetime] = parser.parse(end_date) if end_date is not None else None
        issueSubList = []
        for issue in self.__issue_list:
            issueCreationDate = parser.isoparse(issue.created_date)
            if (startDate is None or issueCreationDate >= startDate) and (endDate is None or issueCreationDate <= endDate):
                issueSubList.append(issue)
        return tuple(issueSubList)
